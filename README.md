...
In Week 1, the fundamentals of the RISC-V SoC tapeout program are introduced. The basics of Verilog RTL design are covered, and the way RTL is translated into gate-level implementations during synthesis is explored. The role of timing libraries is explained, and hierarchical vs flat synthesis approaches are compared to highlight trade-offs in complexity and performance.

Attention is given to efficient flop coding styles, since small differences in RTL are reflected directly in the synthesized netlist. Both combinational and sequential optimizations are studied to show how timing, area, and power can be improved.
